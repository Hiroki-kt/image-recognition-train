...
class _SSDVGG16(chainer.Chain):
    ...
    def __init__(self, labels):
        ...
        super(_SSDVGG16, self).__init__(
            ...
            # mitarai
            norm7=_Normalize(1024, **self.norm_init),
    ...
    def label(self, lbl_id):
        """
        当該モデルのラベルIDからラベルに変換する。
        """
        lblstr = self.labels[lbl_id]
        return lblstr
    ...
    def _features(self, x):
        ...
        if ( hasattr(self, "norm7") ):
            # conv7のレイヤーがVGGの最終層。それ以降SSD層が続き各層の値がsoftmaxされるので、
            # 全SSD層に影響を与えられるのはconv7層まで。なので、ここがNormalizeできる最終層。
            ys.append(self.norm7(h))
        else:
            # original
            ys.append(h)
...
class SSD300(_SSDVGG16):
    ...
    SSD base conv-layer is _SSDVGG16(#convs: 7)
    '''
    insize = 300
    ...
    def __init__(self, labels):
        super(SSD300, self).__init__(labels)
...
class SSD512(_SSDVGG16):
    ...
    def __init__(self, labels):
        super(SSD512, self).__init__(labels)
